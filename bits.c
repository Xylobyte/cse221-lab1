 ...
 */
int thirdBits(void) {
  /* create template of 1001 then overlay it across all 32 bits */
  int template = 9;
  template = (template << 6) | template;
  template = (template << 12) | template;
  template = (template << 9) | 73;
  return template;
}
/* 
...
int bitNor(int x, int y) {
  /* Apply not then join them with an and to make sure both bits are 0 */
  return ~x & ~y;
}
/* Rating 2 */
/* 
...
int allOddBits(int x) {
  /* Create template of ...10101010 across all 32 bits then AND the template 
    with our number to get like bits. Then comare that result to original template.
    A perfect match will satisfy the NOT XOR combo */
  int template = (170 << 8) | 170;
...
int getByte(int x, int n) {
  /* Shifting x n number of bytes over (8 * n bits) then ANDing it with 11111111 to 
    only highlight the bits that matter */
  int block = 255, byte;
  byte = (x >> (n << 3)) & block;
  return byte;
}
...
int leastBitPos(int x) {
  /* By NOTing the word we can set all 0s to 1s. Then we can add 1 to it which should turn
    the 1s to 0s starting from the LSB until it hits a 0 which was originally a 1. This 
    should be the least bit. ANDing this result with the original word should give us a 
    1 at the least position */
  return (~x + 1) & x;
}
/* 
 ...
 */
int sign(int x) {
  /* !!x gets a value of 1 if it's nonzero and sb is the sign of the word. Combining these
    gives -1 if negative, 1 if positive, and 0 if zero */
  int sb = x >> 31;
  return sb | !!x;
}
/* Rating 3 */
